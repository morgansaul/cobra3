#!/usr/bin/env python3
# MEXC Bonus Exploit Framework v13.1 (Debugged)
...
class MEXCBonusHunter:
    ...
    def _get_working_base_url(self):
        """Test which base URL is currently active"""
    ...
    def _generate_web_signature(self, params):
        """Working signature generation for current MEXC API"""
    ...
    def _get_bonus_campaigns(self):
        """Fetch active bonus campaigns with direct claim options"""
        params = {
            "type": "direct_claim",
            "signature": ""
        }
    ...
    def execute_bonus_exploit(self):
        """Working exploit chain with proper error handling"""
        if not (self.base_url := self._get_working_base_url()):
        ...
        # Try each campaign until success
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="MEXC Bonus Hunter v13.1")
    ...
    ██║ ╚═╝ ██║███████╗██╔╝ ██╗╚██████╔╝   ██████╔╝╚██████╔╝██║ ╚████║╚██████╔╝███████║
    ╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝ ╚═════╝    ╚═════╝  ╚═════╝ ╚═╝  ╚═══╝ ╚═════╝ ╚══════╝
    BONUS HUNTER FRAMEWORK v13.1 (DEBUGGED)
    """)
