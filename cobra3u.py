...
class MEXCTester:
    ...
    def test_order(self, symbol, quantity, price):
        """Place test order that checks for valid symbol first"""
        # First check if symbol is valid
        try:
            all_symbols = self.session.get(f"{self.base_url}/api/v3/exchangeInfo").json()
            valid_symbols = [s['symbol'] for s in all_symbols['symbols']]
            
            if symbol not in valid_symbols:
                print(f"❌ Error: Symbol '{symbol}' is invalid. Examples of valid symbols: {valid_symbols[:5]}...")
                return {"error": "Invalid symbol"}
        except Exception as e:
            print(f"⚠️ Failed to validate symbol: {str(e)}")
            return {"error": "Symbol validation failed"}

        # Proceed with order if symbol is valid
        params = {
            'symbol': symbol,
            'side': 'BUY',
            'type': 'LIMIT',
