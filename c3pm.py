#!/usr/bin/env python3
# MEXC Security Team - Quantum Doomsday Testing Framework v9.3 (Fully Tested)
...
class MEXCDoomsdayTester:
    def __init__(self, api_key, api_secret):
        ...
        self._session.headers.update({
            'User-Agent': 'MEXC-SECURITY-TESTER/9.3',
    ...
    async def _test_websocket(self):
        """Fixed WebSocket test with proper timeout handling"""
        try:
            async with websockets.connect(
                self.ws_url,
                subprotocols=["binary"],
                ping_timeout=5,  # Correct timeout parameter
                close_timeout=5
            ) as ws:
                ...
                try:
                    ...
                    return "WebSocket Timeout (no response)"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MEXC Security Tester v9.3')
    ...
    ██║ ╚═╝ ██║███████╗██╔╝ ██╗╚██████╔╝
    ╚═╝     ╚═╝╚══════╝╚═╝  ╚═╝ ╚═════╝ 
    SECURITY TESTING FRAMEWORK v9.3
    """)
